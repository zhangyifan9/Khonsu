...
import cn.edu.nju.irtool.document.Artifact;
                ...
                if (f2.isDirectory()) {
                    for (File f : f2.listFiles()) {
                        if (f.getName().endsWith(postfixName)) {
                            String id = f1.getName() + "&&" + f2.getName() + "&&" + f.getName().split(".txt")[0];
                    ...
                    if (f2.getName().endsWith(postfixName)) {
                        String id = f1.getName() + "&&" + f2.getName().split(".txt")[0];
                        // warning!! jsp文件名 存在“-”字符的编码问题
                        id = id.replace("‐", "-");
                        // System.out.println(id);
                        Artifact artifact = new Artifact(id, FileIOUtil.readFile(f2.getPath()));
                        collections.put(id, artifact);
                    }
                }
