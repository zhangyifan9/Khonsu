...
//		public ModuleRevision addModuleRevision(FeatureRevision[] pos, Feature[] neg);
		...
		public default void extract(Association.Op association) {
			System.out.println("actual extract 开始");
			...
			for (Association.Op origA : originalAssociations) {
				// ASSOCIATION
				// slice the associations. the order matters here! the "left" association's featurs and artifacts are maintained. the "right" association's features and artifacts are replaced by the "left" association's.
		 ...
		 */
		public default Op subset_old(Collection<FeatureRevision> deselected, int maxOrder, EntityFactory entityFactory) {
			...
			Op newRepository = entityFactory.createRepository();
		...
		public default Op subset(Collection<FeatureRevision> deselected, int maxOrder, EntityFactory entityFactory) {
			checkNotNull(deselected);
			checkArgument(maxOrder <= this.getMaxOrder());
			checkNotNull(entityFactory);
		 ...
		 */
		public default Op copy(EntityFactory entityFactory) {
			return this.subset(new ArrayList<>(), this.getMaxOrder(), entityFactory);
		}

		public default void merge(Op otherRepository) {
		 ...
		 */
		public default void merge_old(Op other) {
			checkNotNull(other);
