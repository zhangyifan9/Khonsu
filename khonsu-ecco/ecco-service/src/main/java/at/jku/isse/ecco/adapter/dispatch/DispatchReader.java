...
public class DispatchReader implements ArtifactReader<Path, Set<Node.Op>> {
	...
	protected static final Logger LOGGER = Logger.getLogger(DispatchReader.class.getName());
...
//		return currentReader;
	...
	public Set<Node.Op> read(Path base, Path[] input) {
		...
		for (Path path : input) {
			...
			// let readers read the assigned, modified files，读取文件，拼接文件的节点到目录节点
			for (ArtifactReader<Path, Set<Node.Op>> reader : this.readers) {
				ArrayList<Path> filesList = readerToFilesMap.get(reader);
				System.out.println(reader.getPluginId());

				if(!reader.getPluginId().equals("at.jku.isse.ecco.adapter.plugin.java.JavaPlugin") && filesList != null) {
					for (Path ignoredFile : filesList ){
						System.out.println(base.resolve(ignoredFile) + "被忽略了");
					}
					continue;
				}
				if (filesList != null) {
