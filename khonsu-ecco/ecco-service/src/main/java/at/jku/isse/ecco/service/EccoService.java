...
//	}
	...
	protected Collection<Module> initializeService() {
		...
		if (this.properties.getProperty(ECCO_PROPERTIES_STORAGE) == null) {
			throw new EccoException("No data plugin specified.");//没有指定数据插件
		}
...
//		}
		...
		// artifact adapter modules 组件适配器模块，有text java image
...
//			}
	...
	public synchronized void open() {
		...
		// google.guice框架的入门见https://blog.csdn.net/derekjiang/article/details/7231490
...
//					Repository.Op parentRepository = (Repository.Op) parentService.getRepository();
	...
	public synchronized Commit commit(Configuration configuration) {
		...
		try {
			...
			// jedis.set("dir:"+configuration.toString(), String.valueOf(this.baseDir));
			// jedis.close();
			this.transactionStrategy.end();

			for (Association association: associations) {
				System.out.println(association.computeCondition().getSimpleModuleConditionString() + " " + association.getId());
			}
...
//	public void transAssociationToSingleFeature(Collection<? extends Association> associations){
//		Jedis jedis=new Jedis("localhost",6379);
//		for (Association association : associations) {
//
//		}
...
//	public void setAssociationInRedis(Collection<? extends Association> associations){
//		Jedis jedis=new Jedis("localhost",6379);
//		for (Association association : associations) {
////			MemAssociation association1= (MemAssociation) SerializeUtil.deserialize(SerializeUtil.serizlize(association));
//			jedis.set(association.getId().getBytes(), SerializeUtil.serizlize(association));
//		}
//	}
